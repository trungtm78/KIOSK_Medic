...
class SyndromeClassifier:
    def __init__(
        ...
        project_default = Path(__file__).resolve().parents[2] / "models" / "syndrome_classifier"
        container_default = Path("/app/models/syndrome_classifier")
        fallback_dir = container_default if container_default.exists() else project_default
        base_dir = Path(model_dir or os.getenv("SYNDROME_MODEL_DIR", fallback_dir))
        if not base_dir.exists():
            raise FileNotFoundError(f"Syndrome classifier directory not found: {base_dir}")

        self.config_path = base_dir / "config.json"
        self.classifier_path = base_dir / "syndrome_classifier.joblib"
        ...
        name = model_name or self.config.get("model_name", "AITeamVN/Vietnamese_Embedding")
    ...
    def predict(self, text: str) -> SyndromePrediction:
        if not text or not text.strip():
            return SyndromePrediction(label="unknown", score=0.0, probabilities={})
    ...
    def predict_top(self, text: str, top_k: int = 1) -> List[SyndromePrediction]:
        if not text or not text.strip():
            return []
        top_k = max(1, top_k)
        embedding = self.encoder.encode([text], convert_to_numpy=True, normalize_embeddings=True)
        ...
        results: List[SyndromePrediction] = []
        for idx in ranked:
            ...
            results.append(
                SyndromePrediction(
                    label=label,
                    score=score,
                    probabilities={label: float(probs[i]) for i, label in enumerate(self.classes)},
                )
            )
        return results
