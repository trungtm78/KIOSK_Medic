"""
Train intent classifier using sentence-transformer embeddings + logistic regression, and export to ONNX.
"""
...
def train(
    ...
    intents = [item["intent"] for item in samples]
    ...
    with (output_dir / "intent_classifier.onnx").open("wb") as f:
...
def main():
    parser = argparse.ArgumentParser(description="Train intent classifier and export to ONNX")
    parser.add_argument(
        "--data",
        type=Path,
        default=Path("chat-service/data/intent_training_examples.jsonl"),
        help="Path to JSONL training data",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("chat-service/models/intent_classifier"),
